...
void handleErrors(int n) {
    printf("Houve o erro %d na decriptacao\n", n);
    exit(n);
}
...
int decrypt(unsigned char* ciphertext, int ciphertext_len, unsigned char* key,
     ...
     * this stage.
     */
    if (1 != EVP_DecryptFinal_ex(ctx, plaintext + len, &len)) handleErrors(4);
    plaintext_len += len;
...
int main(int argc, char** argv) {
    ...
    if (keyLen < 16) {
        ...
        memcpy(keyWord, argv[3], 16);
    }

    auto begin = std::chrono::high_resolution_clock::now();

    size_t nEncrypt = decrypt(arqMem, n, keyWord, writeBuffer);
